...
def refiner_network(input_image_tensor):
    ...
    x = layers.Convolution2D(128, 1, 13, border_mode='same', activation='relu',
                             kernel_regularizer=regularizers.l2(1.e-2))(x)
    x = BatchNormalization()(x)
    # x = resnet_block(input_image_tensor, nb_features=128, nb_kernel_rows=3, nb_kernel_cols=3)
...
discriminator.compile(loss=wasserstein_loss, optimizer=d_opt, metrics=['accuracy'])
...
def train_for_n(epochs=50, batch_size=32):
    ...
    for epoch in range(epochs):
        ...
        for i in range(train_steps_per_epoch):
            ...
            X = np.concatenate((X_train, generated_images))
            y = np.concatenate((-np.ones((batch_size, 1)), np.ones((batch_size, 1))))  # class vector
            ...
            d_loss_fake = discriminator.train_on_batch(generated_images, np.ones((batch_size, 1)))
            ...
            # y2[:, 1] = 1
            # for _ in range(5):
            g_loss = GAN.train_on_batch(MC_input, -np.ones((batch_size, 1))) #original y_MC
