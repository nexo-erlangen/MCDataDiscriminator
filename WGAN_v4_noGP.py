...
def build_critic_graph(generator, critic, batch_size=1):
    ...
    return Model(inputs=[generator_in_critic_training, shower_in_critic_training], outputs=[out_critic_training_gen, out_critic_training_shower]), averaged_batch
...
def build_critic():
    ...
    critic.add(LeakyReLU())
    critic.add(Dense(2, activation='softmax'))
    return critic
...
critic_training = critic
critic_training.compile(optimizer=Adam(0.0001, beta_1=0.5, beta_2=0.9, decay=0.0), loss=[wasserstein_loss])
    ...
    for epoch in range(EPOCHS):
        ...
        for i in range(iterations_per_epoch):
            ...
            for j in range(NCR):
                ...
                critic_loss.append(critic_training.train_on_batch([generated_images],
                                                                  [negative_y]))  # train the critic
                critic_loss.append(critic_training.train_on_batch([X_train],
                                                                  [positive_y]))  # train the critic
