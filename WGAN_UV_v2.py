...
def refiner_network(tensor_in):#u_tensor, v_tensor):
    ...
    for i in range(len(tensor_in)):
        ...
        x1 = Conv2D(64, (1, 5), padding='same', activation='relu',
                   kernel_initializer='glorot_uniform', kernel_regularizer=regularizers.l2(1.e-4))(x)
        ...
        y1 = Conv2D(64, (1, 5), padding='same', activation='relu',
                   kernel_initializer='glorot_uniform', kernel_regularizer=regularizers.l2(1.e-4))(x)

        x = layers.merge.add([y1, x1])

        for _ in range(7):
            x = resnet_block(x, 64, nb_kernel_rows=1)
...
def critic_inception(tensor_in):#u_tensor, v_tensor):
    ...
    # print x
    # x = Conv2D(1, (2, 3), kernel_initializer='glorot_uniform', kernel_regularizer=regularizers.l2(1.e-2))(x)
    x = Dense(1)(x)
    # exit()
    return x
