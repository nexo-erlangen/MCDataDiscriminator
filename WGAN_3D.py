...
from keras.layers import ReLU, GlobalMaxPooling2D, Dropout, Activation, MaxPooling2D, MaxPooling3D, Dropout, GlobalAveragePooling2D, Reshape, Concatenate
from keras.layers.convolutional import Convolution2D, Conv2D, Conv2DTranspose, Conv3D
...
def gradient_penalty_loss(y_true, y_pred, averaged_batch, penalty_weight):
    ...
    gradients_sqr_sum = K.sum(K.square(gradients)[0], axis=(1, 2, 3, 4))
...
def build_generator_graph(generator, critic):
    ...
    generator_in = Input(shape=(2, 38, 350, 1))
...
def build_critic_graph(generator, critic, batch_size=1):
    ...
    generator_in_critic_training = Input(shape=(2, 38, 350, 1), name="noise")
    shower_in_critic_training = Input(shape=(2, 38, 350, 1), name='shower_maps')
...
def refiner_network(input_image_tensor):
    ...
    """
    def resnet_block(input_features, nb_features=32, nb_kernel_rows=3, nb_kernel_cols=5, deep=1):
        """
        ...
        """
        y = Conv3D(nb_features, (deep, nb_kernel_rows, nb_kernel_cols), padding='same', activation='relu',
    ...
    x = Conv3D(32, (1, 3, 5), padding='same', activation='relu',
    ...
    y = Conv3D(32, (1, 3, 5), padding='same', activation='relu',
               kernel_initializer='glorot_uniform', kernel_regularizer=regularizers.l2(1.e-4))(input_image_tensor)
    ...
    x1 = Conv3D(64, (1, 3, 5), padding='same', activation='relu',
               kernel_initializer='glorot_uniform', kernel_regularizer=regularizers.l2(1.e-4))(x)

    x1 = Conv3D(64, (1, 3, 5), padding='same', activation='relu',
               kernel_initializer='glorot_uniform', kernel_regularizer=regularizers.l2(1.e-4))(x1)

    y1 = Conv3D(64, (1, 3, 5), padding='same', activation='relu',
               kernel_initializer='glorot_uniform', kernel_regularizer=regularizers.l2(1.e-4))(x)
    ...
    # x = Activation('relu')(x)
    x = Conv3D(1, (1, 1, 1), padding='same', kernel_regularizer=regularizers.l2(1.e-2), name='generator_output')(x)
...
def critic_inception(input_image_tensor):
    ...
    #
    x = Conv3D(64, (1, 3, 5), padding='same', kernel_initializer='glorot_uniform',
                     kernel_regularizer=regularizers.l2(1.e-2))(input_image_tensor)
    # # x = Conv2D(32, (3, 5), padding='same', kernel_initializer='glorot_uniform',
    # #                  kernel_regularizer=regularizers.l2(1.e-2))(input_image_tensor)
    x = LeakyReLU()(x)
    # x = Dropout(0.3)(x)
    x = MaxPooling3D(pool_size=(1, 2, 4))(x)
    #
    x = Conv3D(128, (1, 3, 5), padding='same', kernel_initializer='glorot_uniform',
                     kernel_regularizer=regularizers.l2(1.e-2))(x)
    x = LeakyReLU()(x)
    # x = Dropout(0.3)(x)
    ...
    # # x = MaxPooling2D(pool_size=(2, 4))(x)
    #
    x = Conv3D(256, (1, 3, 5), padding='same', kernel_initializer='glorot_uniform',
                     kernel_regularizer=regularizers.l2(1.e-2))(x)
    x = LeakyReLU()(x)
    # x = Dropout(0.3)(x)
    # # x = MaxPooling2D(pool_size=(2, 4))(x)
...
critic = create_model(model=critic_inception, layer_in=Input(shape=(2, 38, 350, 1)), name='critic')
generator = create_model(model=refiner_network, layer_in=Input(shape=(2, 38, 350, 1)), name='refiner')
...
generator_training.compile(optimizer=Adam(0.00001, beta_1=0.5, beta_2=0.9, decay=0.0), loss=[wasserstein_loss])
...
critic_training.compile(optimizer=Adam(0.00001, beta_1=0.5, beta_2=0.9, decay=0.0), loss=[wasserstein_loss, wasserstein_loss, gradient_penalty])
    ...
    for epoch in range(EPOCHS):
        ...
        for i in range(iterations_per_epoch):
            ...
            # if i % 100 == 0:
                ...
                generated_images = np.reshape(generated_images, (batch_size, 2, 38, 350))
                ...
                for j in range(MC_input.shape[2]):
                    ...
                    plt.subplot(322)
                    plt.plot(generated_images[0,1][j] + j * 20, color='black', linewidth=0.5)
                    plt.title('V Wire')
                    # plt.ylabel('Modified MC')
                    plt.xticks([])
                    plt.yticks([])
                    ...
                    plt.xlim(0, 350)
                    plt.subplot(326)
                    plt.plot(np.subtract(generated_images[0,1][j], MC_input[0,1][j]) + j * 20, color='black', linewidth=0.5)
                    # plt.ylabel('Mod - Orig')
                    plt.xlabel('Time steps')
                    plt.yticks([])
                    plt.xlim(0, 350)
