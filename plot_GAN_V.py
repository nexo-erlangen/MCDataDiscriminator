...
path_load = '/home/vault/capm/mppi060h/MCDataDiscriminator/TrainingRuns/2019_02/2019_02_05-16_09_48/GAN/'
...
path_save = "/home/vault/capm/mppi060h/MCDataDiscriminator/plots_GAN_V/"
...
def plot_loss_acc():
    average_ratio = 500
    loss = np.loadtxt(path_load + '../discriminator_loss.txt', delimiter=',')
    loss_gen = np.loadtxt(path_load + '../generator_loss.txt', delimiter=',')
    acc = np.loadtxt(path_load + '../discriminator_acc.txt', delimiter=',')
    ...
    plt.plot(running_mean(loss, average_ratio), color='blue', markersize=12, label=r'Discriminator Loss')
    # plt.plot(running_mean(loss[:, 1]+loss[:, 2], average_ratio), color='red', label=r'Wasserstein') #, linestyle='dashed')
    # plt.plot(running_mean(loss[:, 3], average_ratio), color='royalblue', markersize=12, label=r'Gradient Penalty', linestyle='dashed')

    plt.plot(running_mean(loss_gen, average_ratio), color='orange', label=r'Generator Loss')
    ...
    plt.xlim(0,)
    # plt.show()
    plt.savefig(path_save + 'averaged_loss.pdf', bbox_inches='tight')
    plt.close()

    plt.plot(running_mean(acc, average_ratio), color='green', markersize=12, label=r'Discriminator Accuracy')
    plt.rcParams.update({'font.size': 14})
    # plt.axhline(0, color='black', linewidth=1)
    plt.grid()
    # plt.legend(loc='best')
    ...
    plt.xlim(0, )
    # plt.show()
    plt.savefig(path_save + 'averaged_acc.pdf', bbox_inches='tight')
    plt.close()
...
generated_images = np.reshape(generated_images, (batch_size, 38, 350))
MC_input = np.reshape(MC_input, (batch_size, 38, 350))
...
for i in range(batch_size):
    ...
    #     else:
        ...
        plt.rcParams.update({'font.size': 15})
        plt.subplot(311)
        plt.plot(MC_input[i][j] + j * 20, color=color, linewidth=0.5)
        # plt.title('V Wire')
        plt.ylabel('Channel')
        plt.yticks([])
        ax2 = plt.twinx()
        ...
        plt.xlim(0, 350)
        plt.subplot(312)
        plt.plot(generated_images[i][j] + j * 20, color=color, linewidth=0.5)
        plt.ylabel('Channel')
        plt.yticks([])
        ax2 = plt.twinx()
        ...
        plt.xlim(0, 350)
        plt.subplot(313)
        plt.plot(np.subtract(generated_images[i][j], MC_input[i][j]) + j * 20, color=color,
                 linewidth=0.5)
        plt.ylabel('Channel')
        plt.yticks([])
    ...
    plt.savefig(path_save + 'wfs_' + '_' + str(i) + '_plot_V_final.pdf', bbox_inches='tight')
    plt.close()
    plt.clf()
