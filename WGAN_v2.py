...
from keras.layers import ReLU, GlobalMaxPooling2D, Dropout, Activation
...
def make_generator():
    model = Sequential()
    model.add(Convolution2D(256, (3, 3), padding='same', input_shape=(76, 350, 1)))
    model.add(ReLU())
...
def make_discriminator():
    ...
    model = Sequential()
    model.add(Convolution2D(64, (3, 3), padding='same', input_shape=(76, 350, 1)))
    model.add(LeakyReLU())
    model.add(Convolution2D(128, (3, 3), kernel_initializer='he_normal',padding='same', strides=[2, 2]))
    model.add(LeakyReLU())
    model.add(Convolution2D(128, (3, 3), kernel_initializer='he_normal', padding='same', strides=[2, 2]))
    model.add(LeakyReLU())
    model.add(Convolution2D(256, (3, 3), kernel_initializer='he_normal', padding='same', strides=[2, 2]))
    model.add(LeakyReLU())
...
real_samples = Input(shape=(76, 350, 1))
generator_input_for_discriminator = Input(shape=(76, 350, 1))
        ...
        for i in range(train_steps_per_epoch):
            ...
            if i % 100 == 0:
                try:
                    ...
                    plt.subplot(2, 1, 1)
                    plt.imshow(np.reshape(generated_images[0], (76, 350)), aspect='auto', cmap=plt.get_cmap('RdBu_r'), norm=colors.Normalize(vmin=-fmax, vmax=fmax))
                    plt.subplot(2, 1, 2)
                    plt.imshow(np.reshape(MC_input[0], (76, 350)), aspect='auto', cmap=plt.get_cmap('RdBu_r'), norm=colors.Normalize(vmin=-fmax, vmax=fmax))
                ...
                for i in range(loss_discriminator.shape[1]):
                    plt.plot(epoch_counter_fine, loss_discriminator[:, i], label='discriminitive loss ' + str(i+1))
                    plt.legend()
                    plt.savefig(path_save + 'loss_discriminitive_'+ str(i+1) +'.pdf')
                    plt.close()
                    plt.clf()
